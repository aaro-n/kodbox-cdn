...
class PathDriverOneDrive extends PathDriverBase {
	...
	public function _init($data = array()){
		...
		foreach($list as $key => $name) {
			if(empty($data[$name])) {
				show_json('OneDrive configuration parameter is abnormal', false);
			}
			$this->$key = $data[$name];
		}
	...
	// curl结果解析
	private function resRequest($res = array()){
		if($res['code'] == '404') return false;
		if(!$res) show_json('network error.', false);
	...
	private function _mkdir($path){
		...
		return $this->graph_request($prefix . '/children', json_encode($param), 'POST', $headers);
	}
	/**
	 * 创建文件夹
	 * @param type $path
	...
	public function copyFile($from,$to) {
		...
		$headers = array('Content-Type: application/json');
		$this->graph_request(':'.$this->pathEncode($from).':/copy', json_encode($param), 'POST', $headers);
		return true;
	}
	...
	public function moveFile($from, $to) {
		...
		$headers = array('Content-Type: application/json');
		$this->graph_request(':'.$this->pathEncode($from), json_encode($param), 'PATCH', $headers);
		return true;
	}
	...
	public function delFile($path) {
		...
		$this->graph_request($path, array(), 'DELETE');
		return true;
	}
	...
	private function recursionFiles($path,&$list, $result,$simple=false) {
		...
		if(isset($result['@odata.nextLink'])) {
			$result = $this->graph_request($result['@odata.nextLink']);
	...
	private function fileList($path){
		...
		$prefix = empty(trim($path, '/')) ? '' : ':'.$this->pathEncode($path).':';
		$limit = 200;
		$param = array('$top' => $limit);
		return $this->graph_request($prefix . '/children?$expand=thumbnails', $param);
		// return $this->graph_request($prefix . '/children', $param);
	}
	...
	private function recursionAllFiles($path,&$list, $result) {
		...
		foreach($fileList as $item) {
			...
			if(!$info['folder'] && $thumb = $this->_thumbUrl($info['path'])) {
				$info['fileThumb'] = $thumb;
			}
			$list[] = $info;
			if(isset($item['folder']['childCount']) && $item['folder']['childCount'] > 0) {
				$this->_clearPathMeta($path);
				$result = $this->fileList($info['path']);
				$this->recursionAllFiles($info['path'],$list, $result);
			}
		}
		if(isset($result['@odata.nextLink'])) {
			$result = $this->graph_request($result['@odata.nextLink']);
	...
	public function setContent($file, $data = '') {
		...
		if(strlen($data) <= 1024*1024*4) {
			...
			$this->graph_request(':'.$this->pathEncode($file).':/content', $data, 'PUT', $headers);
			return true;
		}
		// 创建本地临时文件,并上传
	...
	public function fileSubstr($file, $start, $length) {
		...
		return $this->graph_request(':'.$this->pathEncode($file).':/content', array(), 'GET', $headers);
	}
	...
	public function upload($destPath,$localPath,$moveFile=false,$repeat=REPEAT_REPLACE) {
		...
		// 1.创建上传会话
		$headers = array('Content-Type: application/json');
		$data = $this->graph_request(':'.$this->pathEncode($path) . ':/createUploadSession', array(), 'POST');
	...
	private function uploadFile($url, $data, $content){
		...
		return $this->graph_request($url, $content, 'PUT', $headers);
	}
	...
	private function _thumbLink($path, $width=250){
		...
		$info = $this->graph_request(':'.$this->pathEncode($path).':/thumbnails?select='.$select);
	...
	public function hashMd5($path){
		return md5($this->getContent($path));	// TODO 文件信息中没有MD5
	}
	...
	protected function _objectMeta($path){
		...
		$path = empty(trim($path, '/')) ? '' : ':'.$this->pathEncode($path);
		$info = $this->graph_request($path);
