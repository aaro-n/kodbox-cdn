...
class PathDriverBaidu extends PathDriverBase {
	...
	private function _init($data = array()){
		...
		foreach($list as $key => $name) {
			if(empty($data[$name])) {
				show_json('baidu configuration parameter is abnormal', false);
			}
			$this->$key = $data[$name];
		}
	...
	public function bdRequest($url, $method='GET', $data=false, $headers=false, $options=false, $return=false){
		...
		// write_log(array('百度curl请求结果----', $url, $res, get_caller_info()));
		return $this->resRequest($res, $return);
	}
	// curl结果解析
	private function resRequest($res = array(), $return=false){
		$msg = 'network error.';
		if(!$res) show_json($msg, false);
		if(!$res['status'] && $return) {
			return isset($res['data']) ? $res['data'] : '';
		}
		if(!$res['data']) show_json($msg, false);
		// 获取文件内容
